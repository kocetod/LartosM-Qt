...
#define PLAYM4_FEC_ERR_NOTENABLE				101 // ?????????ì???
#define PLAYM4_FEC_ERR_NOSUBPORT				102 // ??????ç???
#define PLAYM4_FEC_ERR_PARAMNOTINIT				103 // ??ã??????????????
#define PLAYM4_FEC_ERR_SUBPORTOVER				104 // ???????????
#define PLAYM4_FEC_ERR_EFFECTNOTSUPPORT			105 // ?????????????×???????
#define PLAYM4_FEC_ERR_INVALIDWND				106 // ????????
#define PLAYM4_FEC_ERR_PTZOVERFLOW				107 // PTZ??????
...
#define PLAYM4_FEC_ERR_PTZZOOMOVER				113 // PTZ??????????
#define PLAYM4_FEC_ERR_OVERMAXPORT				114 // ?????????????????????????????
#define PLAYM4_FEC_ERR_ENABLED                  115 // ????????????????????
#define PLAYM4_FEC_ERR_D3DACCENOTENABLE			116 // D3D??????ï???-windows?????????-Linux??
#define PLAYM4_FEC_ERR_PLACETYPE                117 // ??????????.????????port?????????????
#define PLAYM4_FEC_ERR_NULLWND                  118 // ????????????hWnd?null????????port????????
#define PLAYM4_FEC_ERR_CorrectType              119 // ???????????,????????,????????port,????PTZ,????????????????·;??????180?????????????????·??
...
#define SYSTEM_MPEG2_PS     0x2				// PS???
#define SYSTEM_MPEG2_TS     0x3				// TS???
#define SYSTEM_RTP          0x4				// rtp???
#define SYSTEM_RTPHIK       0x401				// rtp???

// ???????????
#define VIDEO_NULL          0x0 // ??????
#define VIDEO_H264          0x1 // ???H.264?????H.264???????????????
...
#define AUDIO_MPEG          0x2000 // MPEG ?????????????????????????MPEG???
#define AUDIO_AAC           0x2001 // AAC
#define AUDIO_RAW_DATA8     0x7000 //???????8k????????
#define AUDIO_RAW_UDATA16   0x7001 //???????16k???????????L16
// G??????
#define AUDIO_RAW_DATA8		0x7000      //???????8k????????
#define AUDIO_RAW_UDATA16	0x7001      //???????16k???????????L16
...
{
    unsigned int dwDataType;        //?????????????????????????????Ó?????????????????
    unsigned int dwDataLen;         //??????????????
    unsigned char* pData;           //??????????????????,????IVS_INFO??
} SYNCDATA_INFO;
...
typedef struct _HIK_MEDIAINFO_				// modified by gb 080425
{
    ...
    unsigned short  media_version;			// ?????????????????????0x0101,??1.01????01?????????01????????
    unsigned short  device_id;				// ??ID?????????/????
    ...
    unsigned int    audio_bitrate;          // ??????????,?????bit
...
typedef struct
{
    unsigned char* pDataBuf;      //??????buffer
    unsigned int   dwPicSize;	  //???????Ñ
    unsigned int   dwBufSize;      //????buffer??Ñ
    unsigned int   dwPicWidth;	  //?????
    unsigned int   dwPicHeight;    //?????
    unsigned int   dwReserve;      //??????reserve???
    HKRECT*        pCropRect;     //???????NULL, ?????????
}CROP_PIC_INFO;
#endif
...
typedef enum _PLAYM4_PRIDATA_RENDER
{
    ...
    PLAYM4_RENDER_ADD_POS          = 0x00000004, //POS????????
...
int  PlayM4_GetRefValueEx(int nPort,unsigned char  *pBuffer, unsigned int *pSize);///<???????????
...
int  PlayM4_SetDisplayRegionOnWnd(int nPort,unsigned int nRegionNum, HKRECT *pSrcRect, int bEnable);///<????????
...
PLAYM4_API int __stdcall PlayM4_SetConfigFontPath(int nPort, char* pFontPath);///<?????????????????·??-play?????

//lType: 1 ?????????????PTZ?????????????????????pInfo???plLen??????????;???????pInfo = null???????????????????????plLen
PLAYM4_API int __stdcall PlayM4_GetStreamAdditionalInfo(int nPort, int lType, unsigned char* pInfo, int* plLen);

///<?????
#define R_ANGLE_0   -1  //?????
#define R_ANGLE_L90  0  //???????90??
#define R_ANGLE_R90  1  //???????90??
#define R_ANGLE_180  2  //???180??
...
typedef enum tagFECCorrectType
{
    ...
    FEC_CORRECT_180 = 0x200,		// 180?????  ?????2P??
    FEC_CORRECT_360 = 0x300,		// 360??????? ?????1P??
...
{
    float	fRadiusLeft;	// ????????X????
    float	fRadiusRight;	// ????????X????
    float   fRadiusTop;		// ????????Y????
    float   fRadiusBottom;	// ??????±?Y????

}CYCLEPARAM;

typedef struct tagPTZParam
{
    float fPTZPositionX;		// PTZ ???????????? X????
    float fPTZPositionY;		// PTZ ???????????? Y????
...
typedef struct tagFECParam
{
    ...
    unsigned int	nPlaceAndCorrect;		// ???????????????????????????SetParam???????×,???????????????????????

    PTZPARAM		stPTZParam;				// PTZ Ó???????

    CYCLEPARAM		stCycleParam;			// ?????????????

    float			fZoom;					// PTZ ???????????

    float			fWideScanOffset;		// 180????360??Ó?????????
    FECCOLOR        stPTZColor;             // PTZ???
    int				nResver[15];			// ???????
...
#define    FEC_JPEG   0  // JPEG??
#define    FEC_BMP    1  // BMP ??
...
{
    int moduFlag;      //????????????????PLAYM4_VIE_MODULES?æ???
    //?? PLAYM4_VIE_MODU_ADJ | PLAYM4_VIE_MODU_EHAN
    //?????????????????????????
    //PLAYM4_VIE_MODU_ADJ
    int brightVal;     //??????????[-255, 255]
    int contrastVal;   //??????????[-256, 255]
    int colorVal;      //???????????[-256, 255]
    //PLAYM4_VIE_MODU_EHAN
    int toneScale;     //????????[0, 100]
    int toneGain;      //?????????????????????[-256, 255]
    int toneOffset;    //????????????????????[-255, 255]
    int toneColor;     //??????????????????[-256, 255]
    //PLAYM4_VIE_MODU_DEHAZE
    int dehazeLevel;   //???????[0, 255]
    int dehazeTrans;   //??????[0, 255]
    int dehazeBright;  //?????????[0, 255]
    //PLAYM4_VIE_MODU_DENOISE
    int denoiseLevel;  //???????[0, 255]
    //PLAYM4_VIE_MODU_SHARPEN
    int usmAmount;     //??????[0, 255]
    int usmRadius;     //?????[1, 15]
    int usmThreshold;  //???????[0, 255]
    //PLAYM4_VIE_MODU_DEBLOCK
    int deblockLevel;  //???????[0, 100]
...
///<????????
#define PLAYM4_SOURCE_MODULE             0 // ????????
#define PLAYM4_DEMUX_MODULE              1 // ???????
#define PLAYM4_DECODE_MODULE             2 // ???????
#define PLAYM4_RENDER_MODULE             3 // ??????

#define PLAYM4_RTINFO_SOFTDECODE_ERROR   1 // ?????????
#define PLAYM4_RTINFO_MEDIAHEADER_ERROR  2 // ????????
#define PLAYM4_RTINFO_ALLOC_MEMORY_ERROR 4 // ?????????

typedef struct
{
    int            nRunTimeModule;     //??????????
    int            nStrVersion;        //???????????????????????????,????????????0x0001
    int            nFrameTimeStamp;    //???
    int            nFrameNum;          //????
    int            nErrorCode;         //??????
