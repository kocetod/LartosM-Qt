...
/* Ellipsoid Parameters, must be Airy  */
	BNG_a = 6377563.396;               /* Semi-major axis of ellipsoid, in meters */
	BNG_f = 1 / 299.324964600;         /* Flattening of ellipsoid */

/* BNG projection Parameters */
	BNG_Origin_Lat = (49.0 * PI / 180.0);  /* Latitude of origin, in radians */
	BNG_Origin_Long = (-2.0 * PI / 180.0); /* Longitude of origin, in radians */
	BNG_False_Northing = -100000.0;        /* False northing, in meters */
	BNG_False_Easting = 400000.0;          /* False easting, in meters */
	BNG_Scale_Factor = .9996012717;        /* Scale factor                      */

/* Maximum variance for easting and northing values for Airy. */
	BNG_Max_Easting = 759961.0;
	BNG_Max_Northing = 1257875.0;
	BNG_Min_Easting = -133134.0;
	BNG_Min_Northing = -14829.0;

	memcpy(BNG_Letters, "SV\0", 3);
	BNG_Easting = 0.0;
	BNG_Northing = 0.0;
	string_Broken = 0;
	Airy = "AA";
	memcpy(BNG_Ellipsoid_Code, "AA\0", 3);
...
long CBNG::Find_Index (char letter,
...
long CBNG::Round_BNG (double value)
...
long CBNG::Make_BNG_String (char ltrnum[4],
                      long Easting, 
                      long Northing,
                      char* BNG,
                      long Precision)
...
long CBNG::Check_Out_Of_Area(char BNG500,
                       char BNG100)
...
long CBNG::Break_BNG_String (char* BNG,
                       char Letters[3],
                       double* Easting,
                       double* Northing,
                       long* Precision)
...
long CBNG::Set_BNG_Parameters(char *Ellipsoid_Code)
 ...
 */
  ...
  { /* no errors */
	...
	objTranMerc.Set_Transverse_Mercator_Parameters(BNG_a, BNG_f, BNG_Origin_Lat,
...
void CBNG::Get_BNG_Parameters(char *Ellipsoid_Code)
...
long CBNG::Convert_Geodetic_To_BNG (double Latitude,
                              double Longitude,
                              long Precision,
                              char* BNG)
 ...
 */
  ...
  { /* no errors */
    ...
    Error_Code |= objTranMerc.Convert_Geodetic_To_Transverse_Mercator(Latitude, Longitude, &TMEasting, &TMNorthing);
...
long CBNG::Convert_BNG_To_Geodetic (char *BNG,
                              double *Latitude, 
                              double *Longitude)
 ...
 */
  ...
  if(!Error_Code)
  {
    ...
    if(!Error_Code)
    {
      temp_Error |= objTranMerc.Convert_Transverse_Mercator_To_Geodetic(TMEasting, TMNorthing, Latitude, Longitude);
...
long CBNG::Convert_Transverse_Mercator_To_BNG(double Easting,
                                        double Northing,            
                                        long Precision,
                                        char *BNG)
...
long CBNG::Convert_BNG_To_Transverse_Mercator(char *BNG,
                                        double *Easting,
                                        double *Northing)
