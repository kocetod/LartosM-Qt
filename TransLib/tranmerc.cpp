...
CTranMerc::CTranMerc()
{
	...
	TranMerc_f = 1.0 / 298.257223563;      /* Flattening of ellipsoid  */
...
long CTranMerc::Set_Transverse_Mercator_Parameters(double a,
                    double f,
...
void CTranMerc::Get_Transverse_Mercator_Parameters(double *a,
                                        double *f,
                                        double *Origin_Latitude,
                                        double *Central_Meridian,
                                        double *False_Easting,
...
long CTranMerc::Convert_Geodetic_To_Transverse_Mercator (double Latitude,
                        double Longitude,
                        double *Easting,
                        double *Northing)
...
long CTranMerc::Convert_Transverse_Mercator_To_Geodetic (
                        double Easting,
                        double Northing,
                        double *Latitude,
                        double *Longitude)
...
void CTranMerc::Return_TranMerc_Error_Text(long status, char *String)
{
	*String = 0;
	if((status & TRANMERC_NO_ERROR) == TRANMERC_NO_ERROR)
		strcat(String, "TranMerc not Error | ");
	if((status & TRANMERC_LAT_ERROR) == TRANMERC_LAT_ERROR)
		strcat(String, "TranMerc Longitude outside of valid range (-180 to 360 degrees, and within +/-90 of Central Meridian) | ");
	if((status & TRANMERC_EASTING_ERROR) == TRANMERC_EASTING_ERROR)
		strcat(String, "TranMerc Easting outside of valid range (depending on ellipsoid and projection parameters) | ");
	if((status & TRANMERC_NORTHING_ERROR) == TRANMERC_NORTHING_ERROR)
		strcat(String, "TranMerc Northing outside of valid range (depending on ellipsoid and projection parameters) | ");
	if((status & TRANMERC_ORIGIN_LAT_ERROR) == TRANMERC_ORIGIN_LAT_ERROR)
		strcat(String, "TranMerc Origin latitude outside of valid range (-90 to 90 degrees) | ");
	if((status & TRANMERC_CENT_MER_ERROR) == TRANMERC_CENT_MER_ERROR)
		strcat(String, "TranMerc Central meridian outside of valid range (-180 to 360 degrees) | ");
	if((status & TRANMERC_A_ERROR) == TRANMERC_A_ERROR)
		strcat(String, "TranMerc Semi-major axis less than or equal to zero | ");
	if((status & TRANMERC_INV_F_ERROR) == TRANMERC_INV_F_ERROR)
		strcat(String, "TranMerc Inverse flattening outside of valid range (250 to 350) | ");
	if((status & TRANMERC_SCALE_FACTOR_ERROR) == TRANMERC_SCALE_FACTOR_ERROR)
		strcat(String, "TranMerc Scale factor outside of valid range (0.3 to 3.0) | ");
	if((status & TRANMERC_LON_WARNING) == TRANMERC_LON_WARNING)
		strcat(String, "TranMerc Distortion will result if longitude is more than 9 degrees from the Central Meridian\n");
	strcat(String, "\n");
	return;
}
