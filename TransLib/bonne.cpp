...
/* Ellipsoid Parameters, default to WGS 84 */
	Bonn_a = 6378137.0;                 /* Semi-major axis of ellipsoid in meters */
	Bonn_f = 1 / 298.257223563;         /* Flattening of ellipsoid */
	es2 = 0.0066943799901413800;        /* Eccentricity (0.08181919084262188000) squared  */
	es4 = 4.4814723452405e-005;         /* es2 * es2 */
	es6 = 3.0000678794350e-007;         /* es4 * es2 */
	M1 = 4984944.3782319;               /* Bonn_M(Bonna,c0lat,c1s2lat,c2s4lat,c3s6lat) */
	m1 = .70829317069372;               /* Bonn_m(coslat,sinlat,es2) */
	c0 = .99832429845280;               /* 1 - es2 / 4.0 - 3.0 * es4 / 64.0 - 5.0 * es6 / 256.0 */
	c1 = .0025146070605187;             /* 3.0 * es2 / 8.0 + 3.0 * es4 / 32.0 + 45.0 * es6 / 1024.0 */
	c2 = 2.6390465943377e-006;          /* 15.0 * es4 / 256.0 + 45.0 * es6 / 1024.0 */
	c3 = 3.4180460865959e-009;          /* 35.0 * es6 / 3072.0 */
	a0 = .0025188265843907;             /* 3.0 * e1 / 2.0 - 27.0 * e3 / 32.0 */
	a1 = 3.7009490356205e-006;          /* 21.0 * e2 / 16.0 - 55.0 * e4 / 32.0 */
	a2 = 7.4478137675038e-009;          /* 151.0 * e3 / 96.0 */
	a3 = 1.7035993238596e-011;          /* 1097.0 * e4 / 512.0 */


/* Bonne projection Parameters */
	 Bonn_Origin_Lat = ((45 * PI) / 180.0);           /* Latitude of origin in radians     */
	 Bonn_Origin_Long = 0.0;                          /* Longitude of origin in radians    */
	 Bonn_False_Northing = 0.0;                       /* False northing in meters          */
	 Bonn_False_Easting = 0.0;                        /* False easting in meters           */
	 Sin_Bonn_Origin_Lat = .70710678118655;           /* sin(Bonn_Origin_Lat)              */
	 Bonn_am1sin = 6388838.2901211;                   /* Bonn_a * m1 / Sin_Bonn_Origin_Lat */
...
long CBonne::Set_Bonne_Parameters(double a,
                          double f,
    ...
    if (Bonn_Origin_Lat == 0.0)
		{
			...
			objSinusoidal.Set_Sinusoidal_Parameters(Bonn_a, Bonn_f, Bonn_Origin_Long, Bonn_False_Easting, Bonn_False_Northing);
		}
		else
		{
...
void CBonne::Get_Bonne_Parameters(double *a,
                          double *f,
                          double *Origin_Latitude,
                          double *Central_Meridian,
                          double *False_Easting,
...
long CBonne::Convert_Geodetic_To_Bonne (double Latitude,
                                double Longitude,
                                double *Easting,
                                double *Northing)
 ...
 */
  ...
  { /* no errors */
    if (Bonn_Origin_Lat == 0.0)
			objSinusoidal.Convert_Geodetic_To_Sinusoidal(Latitude, Longitude, Easting, Northing);
		else
		{
...
long CBonne::Convert_Bonne_To_Geodetic(double Easting,
                               double Northing,
                               double *Latitude,
                               double *Longitude)
 ...
 */
  ...
  { /* no errors */
    if (Bonn_Origin_Lat == 0.0)
			objSinusoidal.Convert_Sinusoidal_To_Geodetic(Easting, Northing, Latitude, Longitude);
		else
		{
