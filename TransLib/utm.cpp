...
CUTM::CUTM()
{
	MIN_LAT = ((-80.5 * PI) / 180.0); /* -80.5 degrees in radians    */
	...
	UTM_a = 6378137.0;         /* Semi-major axis of ellipsoid in meters  */
	//UTM_f = 1 / 298.257223563; /* Flattening of ellipsoid                 */
	UTM_f = 1 / 298.257222101; /* Flattening of ellipsoid GRS80                 */
	UTM_Override = 0;          /* Zone override flag                      */
}
...
long CUTM::Set_UTM_Parameters(double a,
                        double f,
                        long   override)
{
/*
...
void CUTM::Get_UTM_Parameters(double *a,
                        double *f,
                        long   *override)
{
/*
...
long CUTM::Convert_Geodetic_To_UTM (double Latitude,
                              double Longitude,
                              long   *Zone,
                              char   *Hemisphere,
                              double *Easting,
  ...
  { /* no errors */
    ...
    if (!Error_Code)
    {
      ...
      *Zone = temp_zone;
	  ...
	  objTranMerc.Set_Transverse_Mercator_Parameters(UTM_a, UTM_f, Origin_Latitude,
                                         Central_Meridian, False_Easting, False_Northing, Scale);
	  objTranMerc.Convert_Geodetic_To_Transverse_Mercator(Latitude, Longitude, Easting,
                                              Northing);
...
long CUTM::Convert_UTM_To_Geodetic(long   Zone,
                             char   Hemisphere,
                             double Easting,
                             double Northing,
                             double *Latitude,
 ...
 */
  ...
  { /* no errors */
    if (Zone >= 31)
      Central_Meridian = ((6 * Zone - 183) * PI / 180.0/* + 0.000000000000005*/);
    else
      Central_Meridian = ((6 * Zone + 177) * PI / 180.0/* + 0.000000000000005*/);
    if (Hemisphere == 'S')
      False_Northing = 10000000;
	CTranMerc objTranMerc;
	objTranMerc.Set_Transverse_Mercator_Parameters(UTM_a, UTM_f, Origin_Latitude,
                                       Central_Meridian, False_Easting, False_Northing, Scale);
	tmpErr = objTranMerc.Convert_Transverse_Mercator_To_Geodetic(Easting,
                                                Northing,
                                                Latitude, 
                                                Longitude);
    if(tmpErr == TRANMERC_LON_WARNING)
		Error_Code |= UTM_WARNING;
	else if(tmpErr)
		Error_Code |= UTM_NORTHING_ERROR;
    if ((*Latitude < MIN_LAT) || (*Latitude > MAX_LAT))
    { /* Latitude out of range */
...
void CUTM::Return_UTM_Error_Text(long status, char *String)
{
	//String[0] = 0;
	//if((status & UTM_NO_ERROR) == UTM_NO_ERROR)
	//	strcat(String,"UTM No errors occurred in function Latitude outside of valid range (-80.5 to 84.5 degrees) | ");
	//if((status & UTM_LON_ERROR) == UTM_LON_ERROR)
	//	strcat(String,"UTM Longitude outside of valid range (-180 to 360 degrees) | ");
	//if((status & UTM_EASTING_ERROR) == UTM_EASTING_ERROR)
	//	strcat(String,"UTM Easting outside of valid range (100,000 to 900,000 meters) | ");
	//if((status & UTM_NORTHING_ERROR) == UTM_NORTHING_ERROR)
	//	strcat(String,"UTM Northing outside of valid range (0 to 10,000,000 meters) | ");
	//if((status & UTM_ZONE_ERROR) == UTM_ZONE_ERROR)
	//	strcat(String,"UTM Zone outside of valid range (1 to 60) | ");
	//if((status & UTM_HEMISPHERE_ERROR) == UTM_HEMISPHERE_ERROR)
	//	strcat(String,"UTM Invalid hemisphere ('N' or 'S') | ");
	//if((status & UTM_ZONE_OVERRIDE_ERROR) == UTM_ZONE_OVERRIDE_ERROR)
	//	strcat(String,"UTM Zone outside of valid range (1 to 60) and within 1 of 'natural' zone | ");
	//if((status & UTM_A_ERROR) == UTM_A_ERROR)
	//	strcat(String,"UTM Semi-major axis less than or equal to zero | ");
	//if((status & UTM_INV_F_ERROR) == UTM_INV_F_ERROR)
	//	strcat(String,"UTM Inverse flattening outside of valid range (250 to 350) | ");
	//if ((status & UTM_WARNING) == UTM_WARNING)
	//	strcat(String, "UTM warning longitude | ");
	//strcat(String, "\n");
	return;
}

long CUTM::UTMZoneFromBL(double B, double L, int *UTMZone)
{
    long Lat_Degrees;
    long Long_Degrees;
    long temp_zone = -1;
    ...
    { /* no errors */
        if (L < 0) L += (2 * PI);
        Lat_Degrees = (long)(B * 180.0 / PI);
        Long_Degrees = (long)(L * 180.0 / PI);

        if (L < PI)
            temp_zone = (long)(31 + ((L * 180.0 / PI) / 6.0));
        else
            temp_zone = (long)(((L * 180.0 / PI) / 6.0) - 29);
        if (temp_zone > 60)
            temp_zone = 1;
        /* UTM special cases */
        if ((Lat_Degrees > 55) && (Lat_Degrees < 64) && (Long_Degrees > -1)
