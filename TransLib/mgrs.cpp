...
CMGRS::CMGRS()
{
	DEGRAD = 0.017453292519943295; /* PI/180                          */
	R3 = 0.052359877559829890; /* RADIANS FOR  3 DEGREES          */
	R8 = 0.139626340159546400; /* RADIANS FOR  8 DEGREES          */
	R9 = 0.157079632679489700; /* RADIANS FOR  9 DEGREES          */
	R21 = 0.366519142918809200; /* RADIANS FOR  21 DEGREES         */
	R33 = 0.575958653158128800; /* RADIANS FOR  33 DEGREES         */
	R56 = 0.977384381116824600; /* RADIANS FOR  56 DEGREES         */
	R64 = 1.117010721276371000; /* RADIANS FOR  64 DEGREES         */
	R72 = 1.256637061435917000; /* RADIANS FOR  72 DEGREES         */
	R80 = 1.396263401595464000; /* RADIANS FOR  80 DEGREES         */
	UPS_SOUTH = 3;    /* UPS COORDINATE IN SOUTHERN HEMISPHERE */
	UPS_NORTH = 2;    /* UPS COORDINATE IN NORTHERN HEMISPHERE */
	UTM = 1;    /* UTM COORDINATE                        */
	//ALBET = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"; /* ALPHABET       */
	strcpy(ALBET, "ABCDEFGHIJKLMNOPQRSTUVWXYZ");
	LETTER_A = 0;   /* ARRAY INDEX FOR LETTER A               */
	LETTER_B = 1;   /* ARRAY INDEX FOR LETTER B               */
	LETTER_C = 2;   /* ARRAY INDEX FOR LETTER C               */
	LETTER_D = 3;   /* ARRAY INDEX FOR LETTER D               */
	LETTER_E = 4;   /* ARRAY INDEX FOR LETTER E               */
	LETTER_H = 7;  /* ARRAY INDEX FOR LETTER H               */
	LETTER_I = 8;   /* ARRAY INDEX FOR LETTER I               */
	LETTER_J = 9;   /* ARRAY INDEX FOR LETTER J               */
	LETTER_L = 11;   /* ARRAY INDEX FOR LETTER L               */
	LETTER_M = 12;   /* ARRAY INDEX FOR LETTER M               */
	LETTER_N = 13;   /* ARRAY INDEX FOR LETTER N               */
	LETTER_O = 14;   /* ARRAY INDEX FOR LETTER O               */
	LETTER_P = 15;   /* ARRAY INDEX FOR LETTER P               */
	LETTER_Q = 16;   /* ARRAY INDEX FOR LETTER Q               */
	LETTER_R = 17;   /* ARRAY INDEX FOR LETTER R               */
	LETTER_S = 18;   /* ARRAY INDEX FOR LETTER S               */
	LETTER_U = 20;   /* ARRAY INDEX FOR LETTER U               */
	LETTER_V = 21;   /* ARRAY INDEX FOR LETTER V               */
	LETTER_W = 22;   /* ARRAY INDEX FOR LETTER W               */
	LETTER_X = 23;   /* ARRAY INDEX FOR LETTER X               */
	LETTER_Y = 24;   /* ARRAY INDEX FOR LETTER Y               */
	LETTER_Z = 25;   /* ARRAY INDEX FOR LETTER Z               */
	RND1 = 0.1e0;  /* ROUND TO NEAREST .1            */
	RND5 = 0.5e0;  /* ROUND TO NEAREST .5            */
	//EOLN = "\0";  /* END OF STRING CHARACTER        */
	*EOLN = char(0);
	//BLANK = " ";  /* BLANK CHARACTER                */
	strcpy(BLANK, " ");
	//MGRS_LETTERS = 3;  /* NUMBER OF LETTERS IN MGRS              */
	NUM_OFFSET = 48;  /* USED TO CONVERT NUMBERS TO LETTERS     */
	ONEHT = 100000.e0;    /* ONE HUNDRED THOUSAND                  */
	TWOMIL = 2000000.e0;    /* TWO MILLION                           */
	EIGHT = 800000.e0;    /* EIGHT HUNDRED THOUSAND                */
	ONE3HT = 1300000.e0;    /* ONE MILLION THREE HUNDRED THOUSAND    */
	ZERO = 0.e0;  /* ZERO                           */
	TEN = 10.e0;    /* TEN                                   */
	//TRUE = 1  /* CONSTANT VALUE FOR TRUE VALUE  */
	//FALSE = 0  /* CONSTANT VALUE FOR FALSE VALUE */
	//PI_OVER_2(PI / 2.0e0)
	//NUM = "01234567890";                /* NUMBERS        */
	strcpy(NUM, "01234567890");
	MAXALBET = 25;   /* LAST INDEX IN ALPHABET ARRAY(0-25)     */
	MAXNUMBER = 10;   /* LAST INDEX IN NUMBER ARRAY(0-9)        */
	MGRS_ZONE_AND_LETTERS = 5;  /* NUM. OF CHARS. IN ZONE AND LETTERS     */
	MGRS_MINIMUM = 9;  /* MINIMUM NUMBER OF CHARS FOR MGRS       */
	MGRS_MAXIMUM = 15;  /* MAXIMUM NUMBER OF CHARS FOR MGRS       */
	...
	MIN_UTM_LAT = ((-80 * PI) / 180.0); /* -80 degrees in radians    */
	MAX_UTM_LAT = ((84 * PI) / 180.0);  /* 84 degrees in radians     */

	MIN_EAST_NORTH = 0;
	MAX_EAST_NORTH = 4000000;

	/* Ellipsoid parameters, default to WGS 84 */
	MGRS_a = 6378137.0;    /* Semi-major axis of ellipsoid in meters */
	MGRS_f = 1.0 / 298.257223563; /* Flattening of ellipsoid           */
	MGRS_recpf = 298.257223563;
	//MGRS_Ellipsoid_Code = "WE";
	strcpy(MGRS_Ellipsoid_Code, "WE");

	//CLARKE_1866 = "CC";
	strcpy(CLARKE_1866, "CC");
	//CLARKE_1880 = "CD";
	strcpy(CLARKE_1880, "CD");
	//BESSEL_1841 = "BR";
	strcpy(BESSEL_1841, "BR");
	/*
...
{ /* BEGIN UTMMGRS */
  ...
  */
  CUTM objUTM;
  objUTM.Set_UTM_Parameters(MGRS_a,MGRS_f,izone);
  objUTM.Convert_Geodetic_To_UTM(spsou, slcm, &izone, &hemisphere, &xltr, &yltr);
...
void CMGRS::LTR2UPS (long* ltrnum,
              long ltrlow, 
              long ltrhi, 
              long ltrhy, 
              long* ierr, 
...
void CMGRS::GRID_UPS (long   *Letters,
               char   *Hemisphere,
               double *Easting,
               double *Northing,
               long   *Error)
{ /* BEGIN GRID_UPS */
  ...
  sphi = spsou;
  CUPS objUPS;
  objUPS.Set_UPS_Parameters(MGRS_a,MGRS_f);
  objUPS.Convert_Geodetic_To_UPS(sphi,slam,Hemisphere,&x,&y);
  spnor = sphi;
  sleast = slam;
  slwest = slam;
...
void CMGRS::LTR2UTM (long* ltrnum,
              long ltrlow, 
              long ltrhi, 
              long* ierr, 
              double *xltr, 
...
{ /* BEGIN GRID_UTM */
  ...
  CUTM objUTM;
  objUTM.Set_UTM_Parameters(MGRS_a,MGRS_f,*Zone);
  slcm = (double)(*Zone * 6 - 183) * DEGRAD; 
  objUTM.Convert_Geodetic_To_UTM(spsou,slcm,Zone,Hemisphere,&xltr,&yltr);
  ...
  *Northing = yltr + *Northing;
  ...
  objUTM.Convert_UTM_To_Geodetic(*Zone,*Hemisphere,*Easting,*Northing,&Latitude,&Longitude);
...
long CMGRS::Round_MGRS (double value)
...
long CMGRS::Make_MGRS_String (char* MGRS,
                       long Zone, 
...
{ /* Make_MGRS_String */
  ...
  long north;
  i = 0;
  //CCHARString strRet = MGRS, strTemp;
  if (Zone)
  {
	  i = sprintf (MGRS+i,"%2.2ld",Zone);
	  //strTemp.Format("%2.2ld", Zone);
  ...
  for (j = 0; j < 3; j++)
  {
	  ...
	  //strTemp = strTemp + ALBET[size_t(ltrnum[j])];
  }
  //strRet = strRet + strTemp;
  ...
  if (Easting >= 99999.5)
    Easting = 99999.0;
  east = Easting/divisor;
  i += sprintf (MGRS+i, "%*.*ld", Precision, Precision, east);
  //strTemp.Format("%*.*ld", Precision, Precision, east);
 // strRet = strRet + strTemp;
  Northing = fmod (Northing, 100000.0);
  if (Northing >= 99999.5)
    Northing = 99999.0;
  north = Northing/divisor;
  i += sprintf (MGRS+i, "%*.*ld", Precision, Precision, north);
  //strTemp.Format("%*.*ld", Precision, Precision, north);
  //strRet = strRet + strTemp;
  //MGRS = strRet.GetCHAR();
  return (error_code);
} /* Make_MGRS_String */
...
long CMGRS::Break_MGRS_String (char* MGRS,
                        long* Zone,
...
{ /* Break_MGRS_String */
  ...
  {
    long n;
    char east_string[6] = { 0 };
    char north_string[6] = { 0 };
    long east;
    long north;
    double multiplier;
...
long CMGRS::Set_MGRS_Parameters (double a,
                          double f,
...
void CMGRS::Get_MGRS_Parameters (double *a,
                          double *f,
                          char* Ellipsoid_Code)
/*
...
{ /* Get_MGRS_Parameters */
  ...
  *f = MGRS_f;
  ...
  //Ellipsoid_Code = MGRS_Ellipsoid_Code.GetCHAR();
  return;
} /* Get_MGRS_Parameters */


long CMGRS::Convert_Geodetic_To_MGRS (double Latitude,
                               double Longitude,
                               long Precision,
                               char* MGRS)
/*
...
{ /* Convert_Geodetic_To_MGRS */
  ...
  if (!error_code)
  {
    if ((Latitude < MIN_UTM_LAT) || (Latitude > MAX_UTM_LAT))
    {
		CUPS objUPS;
		objUPS.Set_UPS_Parameters (MGRS_a, MGRS_f);
      error_code |= objUPS.Convert_Geodetic_To_UPS (Latitude, Longitude, &hemisphere, &easting, &northing);
      error_code |= Convert_UPS_To_MGRS (hemisphere, easting, northing, Precision, MGRS);
    }
    else
    {
		CUTM objUTM;
		objUTM.Set_UTM_Parameters (MGRS_a, MGRS_f, 0);
      error_code |= objUTM.Convert_Geodetic_To_UTM (Latitude, Longitude, &zone, &hemisphere, &easting, &northing);
...
long CMGRS::Convert_MGRS_To_Geodetic (char* MGRS,
                               double *Latitude, 
                               double *Longitude)
/*
 *    MGRS       : MGRS coordinate string           (output)
...
{ /* Convert_MGRS_To_Geodetic */
  ...
  if (!error_code)
    if (Zone)
    {
	  ...
	  objUTM.Set_UTM_Parameters (MGRS_a, MGRS_f, 0);
      error_code |= objUTM.Convert_UTM_To_Geodetic (Zone, Hemisphere, Easting, Northing, Latitude, Longitude);
    }
    else
    {
		CUPS objUPS;
	  ...
	  objUPS.Set_UPS_Parameters (MGRS_a, MGRS_f);
      error_code |= objUPS.Convert_UPS_To_Geodetic (Hemisphere, Easting, Northing, Latitude, Longitude);
    }
  return (error_code);
...
long CMGRS::Convert_UTM_To_MGRS (long Zone,
                          char Hemisphere,
                          double Easting,
                          double Northing,
                          long Precision,
...
{ /* Convert_UTM_To_MGRS */
  ...
  if (!error_code)
  {
	  CUTM objUTM;
	  objUTM.Set_UTM_Parameters (MGRS_a, MGRS_f,0);
    temp_error = objUTM.Convert_UTM_To_Geodetic (Zone, Hemisphere, Easting, Northing, &Latitude, &Longitude);
...
long CMGRS::Convert_MGRS_To_UTM (char   *MGRS,
                          long   *Zone,
                          char   *Hemisphere,
                          double *Easting,
                          double *Northing)
...
long CMGRS::Convert_UPS_To_MGRS (char   Hemisphere,
                          double Easting,
                          double Northing,
                          long   Precision,
                          char*  MGRS)
...
long CMGRS::Convert_MGRS_To_UPS ( char   *MGRS,
                           char   *Hemisphere,
                           double *Easting,
                           double *Northing)
/*
